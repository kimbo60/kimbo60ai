...
# ==========================================
# 🎨 UI 디자인 16.0 (전체 최적화 및 다크모드 완벽 지원)
# ==========================================
st.markdown("""
    <style>
    /* --------------------------------------
       1. 기본 설정 및 타이틀 (테마 자동 적응)
       -------------------------------------- */
    ...
    /* --------------------------------------
       🌙 5. 다크 모드 (Dark Mode) 전용 디자인 🌙
       -------------------------------------- */
    @media (prefers-color-scheme: dark) {
        /* 입력창 및 셀렉트박스를 어두운 회색으로 변경 (요청사항 반영) */
    ...
    @media (max-width: 768px) {
        ...
        /* 🌟 총 살포량 입력창 1줄 고정 (절대 떨어지지 않게 강제 설정) */
...
def load_moa_data():
    try:
        return pd.read_excel('kijak.xlsx').fillna('')
    except:
        return pd.DataFrame()
...
def render_styled_dataframe(df):
    ...
    # 🌟 Styler 배경색 지정 해제 (다크모드/라이트모드 자동 적응)
    styled_df = df.style.set_properties(**{'font-size': '15px', 'font-weight': '600', 'padding': '8px 10px'})
    
    center_cols = [c for c in df.columns if c in ['종류', '상품명', '작용기작', '규격', '사용량']]
...
<a href="/" target="_self" class="home-link">
    <div class='hallabong-title'>{icon_tag} 내가 찾는 농약</div>
</a>
""", unsafe_allow_html=True)
...
if menu == "내가 필요한 농약 찾기":
    ...
    with col_main:
        ...
        with st.form("search_form"):
            ...
            # 모바일 1줄 유지를 위해 column 선언 시 gap="small" 활용
        ...
        if submitted:
            ...
            if not desired_pesticide and not target_pest:
                st.error("⚠️ 희망 약제명 또는 발생 병해충을 하나 이상 입력해 주세요.")
            else:
