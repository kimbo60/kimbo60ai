...
st.markdown("""
    ...
    input[type="text"], div[data-baseweb="select"] span, div[data-baseweb="select"] input, div[data-testid="stDateInput"] input { font-size: 16px !important; padding: 6px 10px !important; }
...
st.markdown(f"<div class='hallabong-title'>{icon_tag} 내가 찾는 농약</div>", unsafe_allow_html=True)
...
if menu == "내가 필요한 농약 찾기":
    ...
    with col_notice:
        st.markdown("""
            <div style='background-color: #fffde7; padding: 25px; border-radius: 15px; border: 2px solid #fdd835; box-shadow: 0px 4px 10px rgba(0,0,0,0.05); height: 100%;'>
                ...
                <ul style='font-size: 16px; color: #333; line-height: 1.8;'>
                    <li><b>[필독]</b> 장마철 검은점무늬병 주의보 발령 (누적 강수량 200mm 초과 예상)</li>
                    <li>[안내] 신규 등록 약제(살균제) 3종 리스트 업데이트 완료</li>
                </ul>
            </div>
        """, unsafe_allow_html=True)
    with col_qa:
        st.markdown("""
            <div style='background-color: #e3f2fd; padding: 25px; border-radius: 15px; border: 2px solid #64b5f6; box-shadow: 0px 4px 10px rgba(0,0,0,0.05); height: 100%;'>
                ...
                <div style='font-size: 16px; color: #333; line-height: 1.6;'>
                    <p>👤 <b>제주농부</b>: 잎 뒷면에 이런 하얀 딱지가 생겼는데 더뎅이병일까요?</p>
                    <p style='margin-left: 15px; color: #1565c0; font-weight: bold;'> └ 👨‍🌾 KIMBO: 사진상으로는 볼록총채벌레 피해 흔적과 유사해 보입니다.</p>
                </div>
            </div>
        """, unsafe_allow_html=True)
